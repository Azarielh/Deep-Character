// Traductions des menus et prompts rapides
...
const setup_content = {
	...
	// Message pour le choix de la fréquence d'envoi
	prompt_frequency: {
		title: '📅 Choix de la fréquence d\'envoi',
		description: `Veuillez choisir la fréquence d\'envoi des prompts automatiques :
1️⃣ Nombre d\'envois (par jour/semaine/mois)
2️⃣ Type de fréquence (quotidienne, hebdomadaire, mensuelle)
3️⃣ Heure d\'envoi (si 2/jour, choisissez deux heures)
\nVous ne pouvez pas choisir plus de 2 envois par jour.`
	},
	...
	en: {
		...
		// Message for prompt frequency selection
		prompt_frequency: {
			title: '📅 Choose prompt sending frequency',
			description: `Please choose the frequency for automatic prompt sending:
				1️⃣ Number of sends (per day/week/month)
				2️⃣ Frequency type (daily, weekly, monthly)
				3️⃣ Sending time (if 2/day, choose two times)
				\nYou cannot select more than 2 sends per day.`
		},
		// Channel selection messages
		channel_selection: {
...
function getSetupEmbed(language, embedType) {
	return setup_content[language]?.[embedType] || setup_content['en'][embedType];
}
